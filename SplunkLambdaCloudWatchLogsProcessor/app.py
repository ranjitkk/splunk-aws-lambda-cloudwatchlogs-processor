...
# 1) Base64 Decode and Gunzip the data
# 2) Parse and extract actual CloudWatch Log data
# 3) Identify if the data is raw data or event data from environment variables
# 4) Add appropriate headers and push data over http HEC input
# 5) If Acknowledgement is enabled, it gets the acknowledgement record and does a asynchronous acknowledgement
# 6) Set Acknowledgement with ACK_REQUIRED env variables and adjust the acknowledgement retries with ACK_RETRIES for number of retries and ACK_WAIT_SECS between ingest 
#    and acknowledgement check
# 7) DEBUG_DATA variable is checked for any data output for debugging

### Data Format
# Raw Payload data (event data) from CW Logs to Lambda
# {'awslogs': {'data': ''}}
# Data after base64 decoded and unpacked (Flow Logs example)
#{'messageType': 'DATA_MESSAGE', 'owner': '647604195155', 'logGroup': 'VPCFlowLogGroup', 'logStream': 'eni-0c19436730cc350ae-all', 'subscriptionFilters': ['flowlogstest'], 'logEvents': [{'id': '37976342262953223179038144714945888722564951965555163136', 'timestamp': 1702918083000, 'message': '2 647604195155 eni-0c19436730cc350ae 10.5.81.140 10.90.0.45 53598 443 6 2 120 1702918083 1702918083 ACCEPT OK'}]}
